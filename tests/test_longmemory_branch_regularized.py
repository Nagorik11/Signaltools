...
def test_long_memory_temporal_head_coupling_operator() -> None:
    features = np.array([
        [1.0 + 0.0j, 0.0 + 1.0j],
        [0.5 + 0.5j, -0.5 + 0.25j],
        [0.0 - 1.0j, 1.0 + 0.0j],
    ...
    out = st.long_memory_temporal_head_coupling_operator(features, frame_size=4, hop_size=2, num_heads=2, memory_decay=0.85)
    assert np.asarray(out.time_real).shape == features.shape
    assert np.asarray(out.attention_weights).shape == (3, 2, 2)
    assert out.meta["long_memory"] is True



def test_attentive_multiscale_stack() -> None:
...
def test_regularized_and_subband_attentive_trees() -> None:
    tensor5d = np.arange(2 * 2 * 4 * 4 * 4, dtype=float).reshape(2, 2, 4, 4, 4)
    reg = st.regularized_adaptive_wavelet_packet_5d_decompose(
        tensor5d,
        level=1,
    ...
    reg_recon = st.regularized_adaptive_wavelet_packet_5d_reconstruct(reg)
    assert np.asarray(reg_recon, dtype=float).ndim == 5
    assert reg.meta["adaptive_regularized"] is True

    att = st.subband_attentive_wavelet_packet_5d_decompose(
        tensor5d,
        level=1,
        candidate_families=["haar", "db8", "coif3", "cdf97"],
        block_shape=(1, 1, 2, 2, 2),
    )
    att_recon = st.subband_attentive_wavelet_packet_5d_reconstruct(att)
    assert np.asarray(att_recon, dtype=float).ndim == 5
    assert att.meta["attentive_subbands"] is True
